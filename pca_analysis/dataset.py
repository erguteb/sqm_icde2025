...
def get_datase(name: str, clipping_norm: float = 1):
    ...
    if os.path.exists(f"dataset/{name}_{clipping_norm}.pkl") is False:
        if name == "iris":
        ...
        elif name in ["cora", "cora_ml", "citeseer", "polblogs"]:
            if os.path.exists(f"data/{name}.pkl") is True:
                _, X, y = pickle.load(open(f"data/{name}.pkl", "rb"))
                X = X.numpy()
                y = y.numpy()
            else:
            ...
            if os.path.exists(f"data/{name}.pkl") is True:
                X, y = pickle.load(open(f"data/{name}.pkl", "rb"))
            else:
                raise ValueError("Dataset not found")
        ...
        logging.info("===========>Shape of y:", y.shape[0])
        return clipped_x, y
       
    else:
        with open(f"dataset/{name}_{clipping_norm}.pkl", "rb") as f:
            datasets = pickle.load(f)
            print("Shape of X:", datasets["X"].shape[0], datasets["X"].shape[1])
